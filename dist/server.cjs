...
async function getGoogleDriveConfig() {
  try {
    ...
    if (rows.length > 0 && rows[0].google_drive_config) {
      const parsed = JSON.parse(rows[0].google_drive_config);
      if (parsed.clientEmail && parsed.privateKey) {
        let formattedKey = parsed.privateKey.trim().replace(/^"|"$/g, "");
        formattedKey = formattedKey.replace(/\\n/g, "\n");
        return {
          clientEmail: parsed.clientEmail,
          privateKey: formattedKey,
          rootFolderGlobal: parsed.rootFolderGlobal || "",
          useDriveForSiswa: parsed.useDriveForSiswa !== false,
          // default true if not set
          useDriveForPendaftar: parsed.useDriveForPendaftar !== false,
          useDriveForPegawai: parsed.useDriveForPegawai !== false,
          useDriveForPerubahanData: parsed.useDriveForPerubahanData !== false
        };
      }
    }
  } catch (err) {
    console.error("Gagal membaca Google Drive config dari DB:", err);
  }
  return null;
}
async function getDriveClient() {
  const config = await getGoogleDriveConfig();
  if (!config) throw new Error("Konfigurasi Google Drive belum diatur di Pengaturan");
...
async function uploadFileToDrive(params) {
  const drive = await getDriveClient();
  const config = await getGoogleDriveConfig();
  if (!config) throw new Error("Konfigurasi Google Drive belum diatur");
  const targetRootFolder = config.rootFolderGlobal;
  if (!targetRootFolder) throw new Error("Root Folder Global belum diatur di Pengaturan");
  const schoolFolderId = await getOrCreateFolder(params.schoolName || "Sekolahku", targetRootFolder);
  let categoryFolderId = await getOrCreateFolder(params.category, schoolFolderId);
  if (params.subfolderName) {
    categoryFolderId = await getOrCreateFolder(params.subfolderName, categoryFolderId);
  }
  const bufferStream = new import_stream.Readable();
  ...
  let viewLink = "";
  if (params.mimeType.startsWith("image/") && uploadRes.data.thumbnailLink) {
    viewLink = uploadRes.data.thumbnailLink.replace(/=s\d+/, "=s1000");
  } else {
    viewLink = uploadRes.data.webViewLink || `https://drive.google.com/file/d/${fileId}/view`;
  }
  return { fileId, viewLink };
}
async function testDriveConnection(config, testFolderId) {
  try {
    let formattedKey = config.privateKey.trim().replace(/^"|"$/g, "");
    formattedKey = formattedKey.replace(/\\n/g, "\n");
    const auth = new import_googleapis.google.auth.JWT({
      email: config.clientEmail,
      key: formattedKey,
    ...
    try {
      if (!testFolderId) throw new Error("No folder ID provided");
      ...
      return { ok: false, error: `Folder tidak ditemukan atau belum di-share ke service account.`, hint: `Pastikan folder Drive (ID: ${testFolderId}) sudah di-share ke: ${config.clientEmail} dengan role Editor` };
    }
    return { ok: true, rootFolderName: folderName };
  } catch (err) {
    const msg = err?.message || "Gagal terhubung ke Google Drive";
    let hint = "";
    if (msg.includes("invalid_grant") || msg.includes("unauthorized")) hint = "Periksa Client Email dan Private Key";
...
app.get("/api/struktur-organisasi/public", asyncHandler(async (req, res) => {
  const [rows] = await getPool().execute(`
        SELECT s.*, p.nama_lengkap, p.nip, COALESCE(f.drive, f.server) AS foto_profil
